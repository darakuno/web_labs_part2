       ...
       if ($_GET['page'] <> 'catalog')
       {
            if ($_SESSION <> [])
            {
                ...
                if (($user["user_type_id"] == 1) || ($user["user_type_id"] == 2))
                {
                    ?> 
                    <div class='a_edit'>
                        <a class="main_text" href="index.php?page=edit&act=edit&book=<?php echo  $_GET['page'] ?>">Редактировать</a>
                        <?php if ($user["user_type_id"] == 1)
                        {
                            ?> <a class="a_del" href="index.php?page=edit_trig&act=del&book=<?php echo  $_GET['page'] ?>">Удалить статью</a> <?php
                        } ?>
                    </div> <?php
                }        
            }
